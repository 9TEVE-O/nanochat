...
# ---------------------------------------------------------------------------
# Arg-parsing — model-tag, step, temperature, top-k
...
def test_model_tag_passed_to_load_model():
    ...
    call_kwargs = mock_checkpoint.load_model.call_args[1]
    assert call_kwargs.get("model_tag") == "v1"
...
def test_step_passed_to_load_model():
    ...
    call_kwargs = mock_checkpoint.load_model.call_args[1]
    assert call_kwargs.get("step") == 500
...
def test_default_model_tag_is_none():
    ...
    _, _, mock_checkpoint, _ = _call_main([], inputs=["quit"])
    call_kwargs = mock_checkpoint.load_model.call_args[1]
    assert call_kwargs.get("model_tag") is None
...
def test_default_step_is_none():
    ...
    _, _, mock_checkpoint, _ = _call_main([], inputs=["quit"])
    call_kwargs = mock_checkpoint.load_model.call_args[1]
    assert call_kwargs.get("step") is None
...
def test_load_model_phase_is_eval():
    ...
    _, _, mock_checkpoint, _ = _call_main([], inputs=["quit"])
    call_kwargs = mock_checkpoint.load_model.call_args[1]
    assert call_kwargs.get("phase") == "eval"
...
def test_temperature_passed_to_generate():
    ...
    call_kwargs = mock_engine_inst.generate.call_args[1]
...
def test_top_k_passed_to_generate():
    ...
    call_kwargs = mock_engine_inst.generate.call_args[1]
    assert call_kwargs.get("top_k") == 100
...
def test_generate_called_with_num_samples_1():
    ...
    _, _, _, mock_engine_inst = _call_main(["--prompt", "Hello"])
    call_kwargs = mock_engine_inst.generate.call_args[1]
    assert call_kwargs.get("num_samples") == 1
...
def test_explicit_device_type_passed_to_compute_init():
    ...
    _, mock_common, *_ = _call_main(["--device-type", "cpu"], inputs=["quit"])
    call_args = mock_common.compute_init.call_args[0]
...
def test_autodetected_device_type_passed_to_compute_init():
    ...
    _, mock_common, *_ = _call_main([], inputs=["quit"])
    # autodetect_device_type returns "cpu" from the mock
    call_args = mock_common.compute_init.call_args[0]
    assert call_args[0] == "cpu"
...
def test_assistant_end_appended_when_missing():
    """When the last generated token is not assistant_end, it is appended before
    extending conversation_tokens. Verified by checking the tokens passed on the
    second turn include the appended assistant_end (999)."""
    sys_modules_patch, _, _, mock_engine_inst = _make_mocks()
    # First call returns token 42 (not assistant_end=999)
    mock_engine_inst.generate.side_effect = [
        iter([([42], None)]),
        iter([([42], None)]),
    ]
    ...
    second_call_tokens = mock_engine_inst.generate.call_args_list[1][0][0]
    # After turn 1: conversation_tokens = [bos=261, user_start=999, 72,101,108, user_end=999,
    #               assistant_start=999, 42 (response), 999 (assistant_end appended)]
    # Turn 2 appends: user_start=999, 72,101,108 (encode "World"), user_end=999, assistant_start=999
    assert 999 in second_call_tokens  # assistant_end was appended
    # The token 42 (response from turn 1) must appear before turn 2's user_start
...
def test_assistant_end_not_duplicated_when_present():
    ...
    sys_modules_patch, _, _, mock_engine_inst = _make_mocks()
    # First call returns assistant_end token (999) as last token
    mock_engine_inst.generate.side_effect = [
        iter([([999], None)]),
        iter([([42], None)]),
    ]
    ...
    second_call_tokens = mock_engine_inst.generate.call_args_list[1][0][0]
    # response_tokens = [999]; last token == assistant_end, so NOT appended again.
    # Count occurrences of 999 in the segment before turn 2's user message.
    # We expect exactly one 999 from the response (not two).
    # Tokens: [261, 999, 72,101,108, 999, 999, 999, 999, 72,101,108, 999, 999]
    # Positions: bos, user_start, encode, user_end, assistant_start, response(999), user_start, ...
    # The response segment should have exactly one 999, not two back-to-back from response.
    # Easier check: total 999 count should be lower than if it were appended (which would add one more).
    count_no_dup = second_call_tokens.count(999)
    # If appended: +1 extra 999. So count_no_dup < count_with_dup.
    # We can compare by re-running with a non-assistant_end response.
    sys_modules_patch2, _, _, mock_engine_inst2 = _make_mocks()
    mock_engine_inst2.generate.side_effect = [
        iter([([42], None)]),   # token 42 → assistant_end (999) appended
        iter([([42], None)]),
    ]
    with ExitStack() as stack2:
        stack2.enter_context(patch.dict("sys.modules", sys_modules_patch2))
        stack2.enter_context(patch("sys.argv", ["nanochat"]))
        stack2.enter_context(patch("builtins.input", side_effect=["Hello", "World", "quit"]))
        from nanochat.__main__ import main as main2
        main2()

    second_call_tokens2 = mock_engine_inst2.generate.call_args_list[1][0][0]
    count_with_dup = second_call_tokens2.count(999)
    # With appending: one extra 999 in conversation
    assert count_no_dup == count_with_dup - 1
...
def test_conversation_tokens_accumulate_across_turns():
    ...
    sys_modules_patch, _, _, mock_engine_inst = _make_mocks()
    mock_engine_inst.generate.side_effect = [
        iter([([77], None)]),   # unique response token for turn 1
        iter([([42], None)]),
    ]
    ...
    second_call_tokens = mock_engine_inst.generate.call_args_list[1][0][0]
    # Token 77 (from turn 1 response) must be in the turn 2 context
    assert 77 in second_call_tokens


def test_tokenizer_decode_called_per_streamed_token():
    """tokenizer.decode() is called once for each token yielded by generate()."""
    sys_modules_patch, _, _, mock_engine_inst = _make_mocks()
    # Yield three tokens in one generate call
    ...
    # Retrieve the tokenizer mock from sys_modules_patch
...
# ---------------------------------------------------------------------------
# Interactive — whitespace handling
...
# ---------------------------------------------------------------------------
# Single-prompt exit prints Goodbye
